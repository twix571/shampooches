...
def fix_superuser(request):
    """
    Temporary endpoint to fix/create superuser from environment variables.

    To use this in production, you must set FIX_SUPERUSER_TOKEN environment variable
    and pass ?token=YOUR_TOKEN in the URL.

    Recommended: Use a randomly generated token that you keep secret.
    ...
    # Check for token authorization in production
    if not settings.DEBUG:
        expected_token = os.getenv('FIX_SUPERUSER_TOKEN', '')
        provided_token = request.GET.get('token', '')
        if not expected_token or expected_token != provided_token:
            return JsonResponse({
                'error': 'Unauthorized. Please provide valid FIX_SUPERUSER_TOKEN as URL parameter.',
                'hint': 'Example: /fix-superuser/?token=YOUR_SECRET_TOKEN'
            }, status=403)
    ...
    email = os.getenv('SUPERUSER_EMAIL', 'admin@shampooches.com')

    if not password:
        return JsonResponse({
            'status': 'error',
            'message': 'SUPERUSER_PASSWORD environment variable not set'
        }, status=400)
    ...
    try:
        if User.objects.filter(username=username).exists():
            ...
            return JsonResponse({
                'status': 'success',
                'message': f'Updated superuser: {username}',
                'username': username,
            ...
            return JsonResponse({
                'status': 'success',
                'message': f'Created superuser: {username}',
                'username': username,
                'is_superuser': user.is_superuser,
                'is_staff': user.is_staff,
                'is_active': user.is_active,
