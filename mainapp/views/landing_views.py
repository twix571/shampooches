...
def admin_landing(request: HttpRequest) -> HttpResponse:
    ...
    today_schedule = Appointment.objects.filter(
        date=today
    ).annotate(cancelled_order=Case(
        When(status='cancelled', then=Value(1)),
        default=Value(0),
        output_field=IntegerField()
    )).order_by('cancelled_order', 'time')

    action_cards = [
        {
            'page_url': 'pricing_management',
            'bg_color': 'bg-[var(--light-gold)]',
            'text_color': 'text-[#D4AF37]',
        ...
        {
            'modal_url': 'legal_agreements_modal',
            'bg_color': 'bg-gray-100',
            'text_color': 'text-[#967D21]',
            'svg_icon': '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>',
            'title': 'Legal Agreements',
            'description': 'Manage contract versions for customer bookings'
        }
    ]
...
def customer_landing(request: HttpRequest) -> HttpResponse:
    ...
    """
    # Redirect groomer users to admin landing
    if request.user.is_authenticated and getattr(request.user, 'user_type', None) == 'groomer':
